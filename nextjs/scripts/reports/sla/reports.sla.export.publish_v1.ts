...
export default async function reportsSlaExportDefault(progress, exportOptions) {
	...
	const allTripAnalysisStream = ridesCollection.find({ operational_date: { $gte: exportOptions.start_date, $lte: exportOptions.end_date } }).stream();
	...
	for await (const tripAnalysisData of allTripAnalysisStream) {
		...
		if (!resultMap.has(`${tripAnalysisData.operational_date}-${tripAnalysisData.line_id}`)) {
			resultMap.set(`${tripAnalysisData.operational_date}-${tripAnalysisData.line_id}`, {
				agency_id: tripAnalysisData.agency_id,
				line_id: tripAnalysisData.line_id,
				operational_date: tripAnalysisData.operational_date,
				total_trip_count: 0,
				pass_trip_count: 0,
			});
		}

		resultMap.get(`${tripAnalysisData.operational_date}-${tripAnalysisData.line_id}`).total_trip_count += 1;

		const simpleOneValidationTransactionTest = tripAnalysisData.analysis.find(item => item._id === 'SIMPLE_ONE_VALIDATION_TRANSACTION');
		const simpleThreeVehicleEventsTest = tripAnalysisData.analysis.find(item => item._id === 'SIMPLE_THREE_VEHICLE_EVENTS');
		...
		if ((simpleOneValidationTransactionTest && simpleOneValidationTransactionTest.grade === 'pass') || (simpleThreeVehicleEventsTest && simpleThreeVehicleEventsTest.grade === 'pass')) {
			resultMap.get(`${tripAnalysisData.operational_date}-${tripAnalysisData.line_id}`).pass_trip_count += 1;
		}
