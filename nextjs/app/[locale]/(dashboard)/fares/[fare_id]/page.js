...
export default function Page() {
  ...
  const isReadOnly = !isAllowed(sessionData, [{ scope: 'fares', action: 'edit' }], { handleError: true }) || fareData?.is_locked;
  ...
  const handleLock = async (value) => {
    try {
      setIsLocking(true);
      await API({ service: 'fares', resourceId: fare_id, operation: 'lock', method: 'PUT' });
      fareMutate();
      setIsLocking(false);
    } catch (err) {
      console.log(err);
