...
export default async function handler(req, res) {
  ...
  try {
    await checkAuthentication({ scope: 'tags', permission: 'create_edit', req, res });
  } catch (err) {
    console.log(err);
  ...
  try {
    parsedData = TagValidation.cast(parsedData);
  } catch (err) {
    console.log(err);
  ...
  try {
    const editedDocument = await TagModel.replaceOne({ _id: { $eq: req.query._id } }, parsedData, { new: true });
