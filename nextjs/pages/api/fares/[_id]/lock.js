...
export default async function handler(req, res) {
  ...
  try {
    ...
    isAllowed(sessionData, [{ scope: 'fares', action: 'lock' }]);
  } catch (error) {
    console.log(error);
  ...
  try {
    const foundDocument = await FareModel.findOne({ _id: { $eq: req.query._id } });
    if (!foundDocument) return await res.status(404).json({ message: `Fare with _id "${req.query._id}" not found.` });
    const updatedDocument = await FareModel.updateOne({ _id: { $eq: foundDocument._id } }, { is_locked: !foundDocument.is_locked }, { new: true });
    return await res.status(200).json(updatedDocument);
  } catch (error) {
    console.log(error);
    return await res.status(500).json({ message: 'Cannot lock or unlock this Fare.' });
  }
