...
export default async function handler(req, res) {
	...
	try {
		...
		const parsedCsvData = Papa.unparse(allMunicipalitiesExportedData, { skipEmptyLines: 'greedy', newline: '\n', header: true });
		await res.writeHead(200, { 'Content-Type': 'text/csv', 'Content-Disposition': `attachment; filename=municipalities.txt` }).send(parsedCsvData);
	} catch (error) {
		console.log(error);
