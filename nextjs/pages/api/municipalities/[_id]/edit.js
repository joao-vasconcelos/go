...
export default async function handler(req, res) {
  ...
  try {
    ...
    isAllowed(sessionData, [{ scope: 'municipalities', action: 'edit' }]);
  } catch (error) {
    console.log(error);
  ...
  try {
    req.body = MunicipalityValidation.cast(req.body);
  } catch (error) {
    console.log(error);
  ...
  try {
    foundDocument = await MunicipalityModel.findOne({ _id: { $eq: req.query._id } });
  ...
  try {
    ...
    const foundDocumentWithMunicipalityCode = await MunicipalityModel.exists({ code: { $eq: req.body.code } });
  ...
  try {
    const editedDocument = await MunicipalityModel.replaceOne({ _id: { $eq: req.query._id } }, req.body, { new: true });
