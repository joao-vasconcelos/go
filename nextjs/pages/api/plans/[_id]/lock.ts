...
export default async function handler(req, res) {
	...
	try {
		...
		isAllowed(sessionData, [{ action: 'lock', scope: 'plans' }]);
	}
	catch (error) {
		console.log(error);
	...
	try {
		...
		if (!foundDocument) return await res.status(404).json({ message: `Plan with _id "${req.query._id}" not found.` });
		...
		console.log(error);
		return await res.status(500).json({ message: 'Cannot lock or unlock this Plan.' });
	}
