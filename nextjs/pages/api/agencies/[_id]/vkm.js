...
export default async function handler(req, res) {
	...
	try {
		await prepareApiEndpoint({ method: 'POST', permissions: [{ action: 'view', scope: 'agencies' }], request: req, session: sessionData });
	}
	catch (error) {
		console.log(error);
			...
			for (const routeData of lineData.routes) {
				for (const patternData of routeData.patterns) {
					for (const scheduleData of patternData.schedules) {
						for (const calendarOnData of scheduleData.calendars_on) {
							dateLoop: for (const dateOn of calendarOnData.dates) {
								...
								for (const calendarOffData of scheduleData.calendars_off) {
