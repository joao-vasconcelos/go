...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const calendarData of allCalendarCodes) {
      //

      const associatedPatternsCalendarsOn = await PatternModel.find({ 'schedules.calendars_on': { $eq: req.query._id } }, '_id code');
      const associatedPatternsCalendarsOff = await PatternModel.find({ 'schedules.calendars_off': { $eq: req.query._id } }, '_id code');

      const associatedPatterns = [...associatedPatternsCalendarsOn, ...associatedPatternsCalendarsOff];

      for (const patternCode of associatedPatterns) {
        //
        ...
        scheduleLoop: for (const scheduleData of patternData.schedules) {
          ...
          for (const calendarOnId of scheduleData.calendars_on) {
            if (calendarOnId === calendarData._id) {
              continue;
            }
