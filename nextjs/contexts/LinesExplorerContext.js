...
export function LinesExplorerContextProvider({ children }) {
  ...
  const { line_id: itemId } = useParams();
  ...
  const { data: allItemsData, isLoading: allItemsLoading, mutate: allItemsMutate } = useSWR('/api/lines');
  const { data: itemData, isLoading: itemLoading, mutate: itemMutate } = useSWR(itemId && `/api/lines/${itemId}`);
  ...
  useEffect(() => {
    ...
    const isReadOnly = !isAllowed(sessionData, [{ scope: 'lines', action: 'edit' }], { handleError: true }) || itemData?.is_locked || pageState.is_saving;
    // Update state
  ...
  useEffect(() => {
    ...
    const populated = populate(LineDefault, itemData);
    // Update form
  ...
  const saveItem = useCallback(async () => {
    try {
      ...
      await API({ service: 'lines', resourceId: itemId, operation: 'edit', method: 'PUT', body: formState.values });
  ...
  const lockItem = useCallback(async () => {
    try {
      await API({ service: 'lines', resourceId: itemId, operation: 'lock', method: 'PUT' });
      itemMutate();
      allItemsMutate();
    } catch (error) {
      itemMutate();
  ...
  const deleteItem = useCallback(async () => {
    try {
      ...
      await API({ service: 'lines', resourceId: itemId, operation: 'delete', method: 'DELETE' });
      router.push('/lines');
      allItemsMutate();
      formState.resetDirty();
    } catch (error) {
