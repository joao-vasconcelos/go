...
export function TypologiesExplorerContextProvider({ children }) {
	...
	useEffect(() => {
		...
		const isReadOnly = !isAllowed(sessionData, [{ action: 'edit', scope: 'typologies' }], { handleError: true }) || itemData?.is_locked || pageState.is_saving;
		// Update state
	...
	const saveItem = useCallback(async () => {
		try {
			...
			await API({ body: formState.values, method: 'PUT', operation: 'edit', resourceId: itemId, service: 'typologies' });
	...
	const lockItem = useCallback(async () => {
		try {
			await API({ method: 'PUT', operation: 'lock', resourceId: itemId, service: 'typologies' });
			itemMutate();
			allItemsMutate();
		}
		catch (error) {
	...
	const deleteItem = useCallback(async () => {
		try {
			...
			await API({ method: 'DELETE', operation: 'delete', resourceId: itemId, service: 'typologies' });
