...
export default function LinesExplorerListHeader() {
	...
	const handleCreate = async () => {
		try {
			...
			const response = await API({ method: 'GET', operation: 'create', service: 'lines' });
			allLinesMutate();
	...
	return (
		<ListHeader>
			<SearchField onChange={linesExplorerContext.updateSearchQuery} query={linesExplorerContext.list.search_query} />
			<Menu position="bottom-end" shadow="md">
				<Menu.Target>
					<ActionIcon color="gray" loading={allLinesLoading || isCreating} size="lg" variant="light">
						<IconDots size={20} />
					</ActionIcon>
				</Menu.Target>
				<Menu.Dropdown>
