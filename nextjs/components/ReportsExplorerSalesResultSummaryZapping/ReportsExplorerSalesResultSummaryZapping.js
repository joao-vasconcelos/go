...
import ReportsExplorerSalesResultSummaryZappingDownload from '@/components/ReportsExplorerSalesResultSummaryZappingDownload/ReportsExplorerSalesResultSummaryZappingDownload';

/* * */

export default function ReportsExplorerSalesResultSummaryZapping() {
  //

  //
  // A. Setup variables

  const t = useTranslations('ReportsExplorerSalesResultSummaryZapping');
  ...
  const metricsDataFormatted = useMemo(() => {
    if (!reportsExplorerSalesContext.request.summary_zapping) return {};
    return {
      total_transactions_qty: reportsExplorerSalesContext.request.summary_zapping.reduce((acc, item) => acc + item.transactions_qty, 0),
      total_sales_qty: reportsExplorerSalesContext.request.summary_zapping.reduce((acc, item) => acc + item.sales_qty, 0),
      total_cashbacks_qty: reportsExplorerSalesContext.request.summary_zapping.reduce((acc, item) => acc + item.cashbacks_qty, 0),
      total_transactions_euro: reportsExplorerSalesContext.request.summary_zapping.reduce((acc, item) => acc + item.transactions_euro, 0) / 100,
      total_sales_euro: reportsExplorerSalesContext.request.summary_zapping.reduce((acc, item) => acc + item.sales_euro, 0) / 100,
      total_cashbacks_euro: reportsExplorerSalesContext.request.summary_zapping.reduce((acc, item) => acc + item.cashbacks_euro, 0) / 100,
    };
  }, [reportsExplorerSalesContext.request.summary_zapping]);
  ...
  const tableDataFormatted = useMemo(() => {
    ...
    return {
      ...
      body: reportsExplorerSalesContext.request.summary_zapping.map((item) => [
        item.product_id,
  ...
  }, [reportsExplorerSalesContext.request.summary_zapping, t]);
  ...
  return (
    <AppLayoutSection title={t('title')} description={t('description')}>
      ...
      <ReportsExplorerSalesResultSummaryZappingDownload />
    </AppLayoutSection>
  );
