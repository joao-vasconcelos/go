...
export default function ReportsExplorerSalesResultSummaryZappingDownload() {
  //

  //
  // A. Setup variables

  const t = useTranslations('ReportsExplorerSalesResultSummaryZappingDownload');
    ...
    <AppAuthenticationCheck permissions={[{ scope: 'reports', action: 'download', fields: [{ key: 'kind', values: ['sales'] }] }]}>
      <div>
        <Tooltip label={reportsExplorerSalesContext.details.is_loading ? t('is_loading') : t('description')} position="bottom" withArrow>
          <Button onClick={reportsExplorerSalesContext.downloadZappingDetail} leftSection={<IconListNumbers size={18} />} loading={reportsExplorerSalesContext.details.is_loading} variant="light" size="xs" color="grape">
            {t('label')}
          </Button>
        </Tooltip>
      </div>
