...
export default function BulkImportFileSelect({ filesParser, onParse }) {
	...
	const ErrorAlert = () => <Alert icon={<IconAlertTriangleFilled size="20px" />} title={t('parsing_error.title')} color="red">
		{hasParsingError || t('parsing_error.description')}
	</Alert>;
	const DropZoneIdle = () => <div className={styles.container} onClick={() => openFileBrowserRef.current()}>
		<IconDragDrop size="60px" />
		<Text size="h1">{t('dropzone.idle.title')}</Text>
		<Text size="h3">{t('dropzone.idle.description')}</Text>
	</div>;
	const DropZoneAccept = () => <Dropzone.Accept>
	...
	const DropZoneReject = () => <Dropzone.Reject>
		<div className={styles.container}>
			<IconBan size="60px" />
