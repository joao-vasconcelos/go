...
class GODB {
  ...
  async connect() {
    try {
      ...
      if (this.mongoClientConnectionInstance && this.mongoClientConnectionInstance.topology && this.mongoClientConnectionInstance.topology.isConnected()) {
        ...
        mongoClientInstance = await MongoClient.connect(process.env.GODB_MONGODB_URI, mongoClientOptions);
      }
      ...
      if (this.mongoClientConnectionRetries < MAX_CONNECTION_RETRIES) {
        console.error(`GODB: Error creating MongoDB Client instance ["${error.message}"]. Retrying (${this.mongoClientConnectionRetries}/${MAX_CONNECTION_RETRIES})...`);
        await this.reset();
        await this.connect();
      } else {
        console.error('GODB: Error creating MongoDB Client instance:', error);
        await this.reset();
      }
    }
  ...
  async reset() {
    ...
    console.log('GODB: Reset connection.');
  }
