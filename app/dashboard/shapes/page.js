...
import { Tooltip, Textarea, Table, SimpleGrid, TextInput, Code, ActionIcon, Divider, Group, Text, useMantineTheme, rem, Button, Alert } from '@mantine/core';
...
export default function Page(props) {
  ...
  const handleImportInit = () => {
    openConfirmModal({
      ...
      onConfirm: async () => {
        for (const currentShape of shapesToImport) {
          try {
    ...
    >
      ...
      <Section>
        ...
        <SimpleGrid cols={2} pb={'lg'}>
          <Button onClick={handleImportInit} disabled={!shapesToImport.length}>
            {`Iniciar Importação (${shapesToImport.length} shapes)`}
          </Button>
          <Button color='red' onClick={handleImportCancel} disabled={!shapesToImport.length}>
