...
export default async function handler(req, res) {
  ...
  try {
    parsedData = FareValidation.cast(parsedData);
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    const foundDocumentWithFareCode = await FareModel.exists({ code: { $eq: parsedData.code } });
  ...
  try {
    const editedDocument = await FareModel.updateOne({ _id: { $eq: req.query._id } }, parsedData, { new: true });
