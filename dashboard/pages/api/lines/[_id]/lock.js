...
export default async function handler(req, res) {
  ...
  try {
    await checkAuthentication({ scope: 'lines', permission: 'lock', req, res });
  } catch (err) {
    console.log(err);
  ...
  try {
    const foundDocument = await LineModel.findOneAndUpdate({ _id: { $eq: req.query._id } }, { is_locked: req.body.is_locked ? true : false }, { new: true });
