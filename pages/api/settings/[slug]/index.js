...
export default async function settingsGet(req, res) {
  //
  await delay();
  ...
  try {
    const foundDocument = await Model.findOne({ slug: req.query.slug });
    if (!foundDocument)
    ...
    console.log(err);
    return await res.status(500).json({ message: 'Cannot fetch this Setting.' });
  }
}
