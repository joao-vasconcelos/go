...
import { Validation, Model } from '../../../../schemas/surveys/documents';
...
export default async function surveysEdit(req, res) {
  ...
  try {
    ...
    if (req.body.unique_code) {
      ...
      if (foundUniqueCode && foundUniqueCode._id != req.query._id)
        throw new Error('A Survey with the same unique code already exists.');
    }
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    console.log(err);
    return await res.status(500).json({ message: 'Cannot update this Survey.' });
  }
}
