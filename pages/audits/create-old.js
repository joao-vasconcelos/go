...
export default function AuditCreate() {
  ...
  const handleSave = async (values) => {
    try {
      ...
      const response = await API({ service: 'audits', operation: 'create', method: 'POST', body: values });
      router.push(`/audits/${response._id}`);
      notify('new', 'success', 'Audit created successfully!');
    } catch (err) {
      console.log(err);
      setIsLoading(false);
  ...
  return (
    <form onSubmit={form.onSubmit(handleSave)}>
      <PageContainer title={`Audits › New Audit ${form.values.unique_code && `(${form.values.unique_code})`}`}>
        ...
        <Pannel title={'Compliance'}>
          <TextInput label={'Unique Code'} placeholder={'000000'} {...form.getInputProps('unique_code')} disabled />
        </Pannel>
